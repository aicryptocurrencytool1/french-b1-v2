...
const Phrases: React.FC<PhrasesProps> = ({ language }) => {
  ...
  useEffect(() => {
    ...
    const fetchPhrases = async () => {
      setLoading(true);
      const data = await getDailyPhrases(selectedTopic, currentTense, language);
      setPhrases(data);
      setLoading(false);
    };
  ...
  useEffect(() => {
      if (!availableTenses.includes(selectedTense)) {
          return; // Avoid fetching if tense is about to be updated by topic change
      }
      const fetchPhrases = async () => {
        setLoading(true);
